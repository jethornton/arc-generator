...
Open in Axis and you can send the ouput directy to Axis.
...
class Application(Frame):
  ...
  def createWidgets(self):
    ...
    if IN_AXIS:
      ...
      self.sendPreamble = Button(self, text='Send Preamble', command=self.SendPreamble)
      self.sendPreamble.grid(row=13, column=0, sticky=W)
      self.sendAllButton = Button(self, text='Send All', command=self.SendAll)
      self.sendAllButton.grid(row=13, column=0, sticky=E)
      self.sendArcButton = Button(self, text='Send Arc', command=self.SendArc)
      self.sendArcButton.grid(row=13, column=0)

    self.quitButton.grid(row=13, column=1, sticky=E)
  ...
  def DoIt(self):
      ...
      if self.ArcEnd <= 90: # Quadrant 1
          ...
          self.ZEnd = self.YArcCenterN + (self.ArcRadius * sin(radians(self.ArcEnd)))
  ...
  def SendPreamble(self):
    sys.stdout.write('G8 G20 G18 G40 G49 G64 P0.005 G80 G90 G94')
    self.quit()
